...
def getBaseInfo(logdate):
    ...
    outputpath = "hdfs://szwg-ecomon-hdfs.dmop.baidu.com:54310/app/ps/rank/ubs/sefe/wangbo28/%s_wise_gbs_baseinfo_out" % logdate
    ...
    command =  basedir + "/bin/hadoop streaming " + \
        ...
        " -jobconf mapred.job.name=\"ps_fe_wangbo28_wise_gbs_baseinfo_%s\"" % logdate + \
...
def calculate(logdate):
    basedir = "/home/work/hadoop/hadoop"
    inputpath = "hdfs://szwg-ecomon-hdfs.dmop.baidu.com:54310/app/ps/rank/ubs/sefe/wangbo28/%s_wise_gbs_baseinfo_out/part-*" % logdate
    ...
    outputpath = "hdfs://szwg-ecomon-hdfs.dmop.baidu.com:54310/app/ps/rank/ubs/sefe/wangbo28/%s_wise_gbs_calculate_out" % logdate
    ...
    command =  basedir + "/bin/hadoop streaming " + \
        ...
        " -jobconf mapred.job.reduce.capacity=400" + \
        " -jobconf mapred.job.map.capacity=800" + \
        " -jobconf mapred.job.name=\"ps_fe_wangbo28_wise_gbs_calculate_%s\"" % logdate + \
        " -jobconf mapred.job.priority=VERY_HIGH"
    print command
    os.system(command)
...
def saveResult(logdate):
    ...
    inputpath = "hdfs://szwg-ecomon-hdfs.dmop.baidu.com:54310/app/ps/rank/ubs/sefe/wangbo28/%s_wise_gbs_calculate_out/part-00000" % logdate

    year = logdate[0:4]
    month = logdate[4:6]
    outputpath = "%s/../../data/data_ua/%s/%s" % (currdir, year, month)    
